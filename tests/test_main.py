...
class TestGenerateMetadataFile(unittest.TestCase):
    def test_generate_metadata_file_invalid_version(self):
        ...
        with self.assertRaises(SystemExit) as cm1, \
            ...
            target.generate_metadata_file(
                "application_id", "run_description", "oldest_date",
                "most_recent_date", "billing_type", "num_input_rows", "run_id",
                file_version)
    ...
    def test_generate_metadata_file(self):
        ...
        output = target.generate_metadata_file(
            application_id, run_description, oldest_date, most_recent_date,
            billing_type, num_input_rows, run_id, file_version)
        self.assertEqual(
            output,
            "SAABB                            "
...
class TestParseArgs(unittest.TestCase):
    ...
    def test_parse_args_valid_arguments(self):
        ...
        parser = target.parse_args([
            "--input", input_file, "--run-id", "123", "--config", config_file,
            "--application-id", "SE"])
    ...
    def test_parse_args_debug(self):
        ...
        with self.assertLogs(level='DEBUG') as cm:
            parser = target.parse_args([
                "--input", input_file, "--output-directory", output_directory,
                "--config", config_file, "--debug", "--application-id", "SE",
                "--run-id", "123"])
    ...
    def test_parse_args_application_id_str(self):
        ...
        with self.assertRaises(SystemExit) as cm1, \
            ...
            target.parse_args([
                "--input", input_file, "--output-directory", output_directory,
                "--config", config_file, "--application-id", "INVALID",
                "--run-id", "123"])
    ...
    def test_parse_args_billing_type_invalid(self):
        ...
        with self.assertRaises(SystemExit) as cm1, \
            ...
            target.parse_args([
                "--input", input_file, "--output-directory", output_directory,
                "--config", config_file, "--application-id", "AA",
                "--billing-type", "INVALID", "--run-id", "123"])
    ...
    def test_parse_args_run_id_str(self):
        ...
        self.assertEqual(cm2.output, [
            'CRITICAL:root:The `--run-id` argument must be numeric. '
            'Exiting...'])
    ...
    def test_parse_args_file_version_invalid(self):
        ...
        with self.assertRaises(SystemExit) as cm1, \
            ...
            target.parse_args([
                "--input", input_file, "--output-directory", output_directory,
                "--config", config_file, "--application-id", "SE",
                "--run-id", "123", "--file-version", "INVALID"])
...
class TestLicense(unittest.TestCase):
    ...
    def test_license_mention(self):
        ...
        with open('billingflatfile.py') as f:
            ...
            # Confirm it is the MIT License
            self.assertTrue(
                "#    This file is part of billingflatfile and is "
                "MIT-licensed." in s)
